...
Mahoney 互补滤波算法是一种成熟的姿态估计算法, 旨在融合多种传感器的数据来估计最优姿态. 本篇文章将介绍这一滤波器的数学推导与应用.
...
对于四元数运动学, 因为四元数只有 $4$ 个自由度, 冗余的自由度不足以优化旋转所需的 $3$ 个自由度, 我们只能选取一个参量来进行优化. 通常而言选择约束转角.
